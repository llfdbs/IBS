...
public class SendedTaskListFrag extends Fragment implements
	...
	Handler handler_unfinish = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case 0:
				...
				adapter = new SendTaskListAdapter(context, task_unfinishList_data);
				mListView.setAdapter(adapter);
				adapter.notifyDataSetChanged();
				mPullListView.onPullDownRefreshComplete();
				mPullListView.onPullUpRefreshComplete();
		...
		public void handleMessage(Message msg) {
			...
			case 0:
				if (null != msg.obj) {
					...
					adapter = new SendTaskListAdapter(context, task_unsendList_data);
					mListView.setAdapter(adapter);
					adapter.notifyDataSetChanged();
					mPullListView.onPullDownRefreshComplete();
					mPullListView.onPullUpRefreshComplete();
	...
	public void onPullDownToRefresh(PullToRefreshBase<ListView> refreshView) {
		...
		case Container.MODEL_UNSEND:
			...
			pageno_finish = 1;
			setPage(task_finishPage, 1, 1, pageSize);
			initHandler(handler_unsend, "0", task_finishPage);
			break;
		}
	}
