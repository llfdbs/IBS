...
public abstract class SQLiteDALBase<M> implements SQliteBaseinter {
	...
	protected long getPrimaryFieldValue(M pModelObject) {
		try {
			Field _PrimaryField = mModelClass.getDeclaredField(mColumnFieldMap
	...
	public M select(long pID) {
		...
		List<M> _ModelList = select(_PrimaryKeyName + "=" + pID, null, null,
				null, null);
	...
	public boolean insert(M pModelObject) {
		long _RowID = mSQLiteDatabase.insert(mTableName, null,
				toContentValues(pModelObject));
	...
	public boolean update(M pModelObject) {
		...
		beginTransaction();
		try {
			int _UpdateRowNumbers = update(pModelObject, _PrimaryKeyName + "="
	...
	protected List<M> select(String pSelection, String[] pSelectionArgs,
			String pGroupBy, String pHaving, String pOrderBy) {
		List<M> _ModelObjectList = new ArrayList<M>();
		Cursor _Cursor = mSQLiteDatabase.query(mTableName, mAllColumnName,
				pSelection, pSelectionArgs, pGroupBy, pHaving, pOrderBy);
	...
	protected int getCount(String pSelection, String[] pSelectionArgs) {
		Cursor _Cursor = this.getSqLiteDatabase().query(mTableName,
				mAllColumnName, pSelection, pSelectionArgs, null, null, null);
		int _Count = _Cursor.getCount();
		_Cursor.close();
		return _Count;
	}

	protected int update(M pModelObject, String pSelection,
			String[] pSelectionArgs) {
		return mSQLiteDatabase.update(mTableName,
				toContentValues(pModelObject), pSelection, pSelectionArgs);
	}

	protected int delete(String pSelection, String[] pSelectionArgs) {
		return this.getSqLiteDatabase().delete(mTableName, pSelection,
				pSelectionArgs);
	}
	...
	/**
	 * Âº?ßãÊï∞ÊçÆÂ∫ì‰∫ãÂä°„? Transactions can be nested. When the outer transaction is
	 * ended all of the work done in that transaction and all of the nested
	 * transactions will be committed or rolled back. The changes will be rolled
	 * back if any transaction is ended without being marked as clean (by
